...
float plotX(vec2 st, float pct){
  return  smoothstep( pct-0.002, pct, st.x+0.008) -
          smoothstep( pct, pct+0.002, st.x+0.008);
}
...
void main() {
    ...
    st *= vec2( 1.0 + s * (0.1*u_slot1) ); //// multiply the uv coord for 1 + the noise
    ...
    if(uv.y>0.5){
        ...
        float pctY = plotY(uv,st.x/2.);
        float pctX = plotX(uv,st.y*2.);
    	color = pctX*vec3(1.,0.,0.) + pctY*vec3(0.0,1.0,0.0);
    }
